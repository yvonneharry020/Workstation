...
const TYPE_STYLES: Record<NotifType, { bg: string; text: string; border: string; label: string }> = {
  ...
  announcement: { bg: 'bg-admin-900/20',  text: 'text-admin-400',  border: 'border-admin-800/30',   label: 'Announcement' },
}
                ...
                <div className="flex gap-2">
                  {(['all','candidates','companies'] as const).map(t => (
                    <button key={t} onClick={() => setForm(f => ({ ...f, target: t }))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${form.target === t ? 'bg-admin-500 text-white border-admin-500' : 'bg-surface-elevated border-surface-border text-text-secondary'}`}>
                      {TARGET_LABELS[t]}
                    </button>
                  ))}
                </div>
