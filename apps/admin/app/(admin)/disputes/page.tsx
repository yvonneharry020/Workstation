...
type DisputeStatus = 'open' | 'investigating' | 'upheld' | 'badge_removed' | 'info_requested'
        ...
        <div className="px-6 py-5 space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    value={form.badgePeriod}
                    onChange={(e) => set('badgePeriod', e.target.value)}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Summarise what the user reported in the live chat or ticket…"
                  rows={4}
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-colors capitalize ${
                        form.priority === p
                          ...
                          : 'border-surface-border text-text-secondary hover:border-admin-600'
                      }`}
                    >
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Paste file URLs from live chat attachments, one per line…"
              rows={3}
...
const ACTION_META: Record<ActionType, { label: string; desc: string; btnClass: string; newStatus: DisputeStatus }> = {
  uphold: {
    label: 'Uphold Badge',
...
export default function BadgeDisputePage() {
  ...
  const resolved: DisputeStatus[] = ['upheld', 'badge_removed']
  ...
  const counts = {
    ...
    investigating: disputes.filter((d) => d.status === 'investigating' || d.status === 'info_requested').length,
    resolved: disputes.filter((d) => resolved.includes(d.status)).length,
  }
  ...
  return (
    <div className="flex flex-col min-h-full">
      <TopBar
        ...
        subtitle={`${disputes.length} total · ${counts.open} open · ${counts.investigating} under investigation`}
      />
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((dispute) => {
              ...
              const isResolved = resolved.includes(dispute.status)
                    ...
                    <div className="border-t border-surface-border px-5 py-5 space-y-5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2.5">
                          ...
                          {dispute.company_name && (
                            <Row label="Company" value={dispute.company_name} />
                          )}
                          {dispute.role_held && (
                            <Row label="Role" value={dispute.role_held} />
                          )}
                          {dispute.badge_period && (
                            <Row label="Period" value={dispute.badge_period} />
                          )}
                          {dispute.candidate_email && (
                            <Row label="Contact" value={dispute.candidate_email} mono />
                          )}
                        </div>
                        <div>
                      ...
                      {!isResolved && (
                        <div className="pt-3 border-t border-surface-border space-y-3">
                          {dispute.status === 'open' && (
                            <button
                              onClick={() => void startInvestigating(dispute.id)}
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => setActionModal({ id: dispute.id, name: dispute.candidate_name, action: 'uphold' })}
                            ...
                            <button
                              onClick={() => setActionModal({ id: dispute.id, name: dispute.candidate_name, action: 'remove' })}
                            ...
                            <button
                              onClick={() => setActionModal({ id: dispute.id, name: dispute.candidate_name, action: 'info' })}
      ...
      {actionModal && (
        <ActionModal
          disputeId={actionModal.id}
          candidateName={actionModal.name}
