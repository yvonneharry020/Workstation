...
const STAFF_OPTIONS = [
  'yvonne2okis@gmail.com',
  'support@workstation.ng',
  'tech@workstation.ng',
  'billing@workstation.ng',
  'verification@workstation.ng',
]

const DEPARTMENT_OPTIONS = ['Support', 'Technical', 'Billing', 'Verification', 'Management']
...
export default function SupportTicketsPage() {
  ...
  async function handleSelectTicket(id: string) {
    ...
    // Auto-mark in_progress when admin opens a new ticket
  ...
  async function handleResolve() {
    ...
    setTickets(prev => prev.map(t => t.id === selectedId
      ? { ...t, status: 'resolved', resolved_at: new Date().toISOString(), resolution_note: resolutionNote.trim() || null }
      : t
    ))
    const { data: { user } } = await supabase.auth.getUser()
    await supabase.from('audit_logs').insert({
  ...
  async function handleCreateTicket() {
    ...
    if (!error && data) {
      await supabase.from('audit_logs').insert({
        ...
        metadata: { assigned_to: createForm.assigned_to, department: createForm.department },
      })
      setShowCreate(false)
            ...
            {loading ? (
              ...
              return (
                <button key={t.id} onClick={() => handleSelectTicket(t.id)}
                  className={`w-full text-left px-4 py-3.5 transition-colors ${isActive ? 'bg-admin-900/40 border-l-2 border-l-admin-500' : 'hover:bg-surface-elevated/40 border-l-2 border-l-transparent'}`}>
          ...
          {!selected ? (
            <div className="h-full flex items-center justify-center text-text-muted">
              <div className="text-center">
                ...
                <p className="text-xs mt-1">Opening a new ticket automatically marks it In Progress</p>
              </div>
            </div>
          ) : (
            <div className="p-6 space-y-5 max-w-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-text-secondary mt-0.5">
                    {selected.submitter_name} · {selected.submitter_email} · Submitted {formatTime(selected.created_at)}
                  </p>
                </div>
              ...
              {selected.status !== 'resolved' && selected.status !== 'closed' && (
                <div className="bg-surface-card rounded-xl border border-surface-border p-4">
                  <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">
                    Resolution Note <span className="text-text-muted font-normal normal-case">(optional · shown to user)</span>
                  </p>
              ...
              <div className="bg-surface-card rounded-xl border border-surface-border p-4">
                <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Assign To</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={selected.assigned_to ?? ''} onChange={e => handleAssign(e.target.value, selected.department ?? '')}
                      className="w-full bg-surface-elevated border border-surface-border rounded-lg px-3 py-2 text-sm text-text-primary focus:border-admin-500 focus:outline-none">
              ...
              <div className="bg-surface-card rounded-xl border border-surface-border p-4">
                <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">
                  Internal Notes <span className="text-text-muted font-normal normal-case">(not shown to user)</span>
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-surface-card border border-surface-border rounded-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-4">
              <p className="text-xs text-text-muted">Create a ticket on behalf of a user and assign it to the right team member.</p>

              <div>
                <label className="text-xs font-semibold text-text-secondary mb-1.5 block">User type</label>
                <div className="flex gap-2">
                  {(['candidate','company'] as const).map(t => (
                    <button key={t} onClick={() => setCreateForm(f => ({ ...f, submitter_type: t }))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize border transition-colors ${createForm.submitter_type === t ? 'bg-admin-500 text-white border-admin-500' : 'bg-surface-elevated text-text-secondary border-surface-border'}`}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={createForm.description} onChange={e => setCreateForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe the issue in detail…" rows={4}
                  className="w-full bg-surface-elevated border border-surface-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-admin-500 focus:outline-none resize-none" />
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={() => setShowCreate(false)} className="flex-1 py-2.5 rounded-xl bg-surface-elevated border border-surface-border text-sm font-semibold text-text-secondary hover:text-text-primary transition-colors">
                  Cancel
                </button>
                <button onClick={handleCreateTicket} disabled={creating || !createForm.subject.trim() || !createForm.description.trim() || !createForm.submitter_email.trim()}
                  className="flex-1 py-2.5 rounded-xl bg-admin-500 hover:bg-admin-600 text-white text-sm font-semibold disabled:opacity-40 transition-colors">
