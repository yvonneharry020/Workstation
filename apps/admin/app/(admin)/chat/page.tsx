...
export default function LiveChatPage() {
  ...
  // Real-time: new threads and thread updates
  useEffect(() => {
    const ch = supabase
      .channel('chat-threads-admin')
  ...
  // Real-time: new messages in selected thread
  useEffect(() => {
    if (!selectedId) return
    const ch = supabase
  ...
  async function handleSendReply() {
    ...
    if (!error) {
      await supabase.from('chat_threads').update({
        ...
        status: 'in_progress',
        assigned_to: 'admin@workstation.ng',
                ...
                <button key={t.id} onClick={() => setSelectedId(t.id)}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: `${typeMeta.color}20`, color: typeMeta.color }}>
                        {typeMeta.label}
                      </span>
          ...
          {!selected ? (
            ...
            <>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                {messages.length === 0 && (
                  <div className="text-center text-text-muted text-sm py-8">No messages yet. Send the first reply.</div>
                )}
                {messages.map(msg => {
                ...
                <div className="px-6 py-4 border-t border-surface-border">
                  <div className="flex gap-3">
                    <textarea
                      ...
                      rows={2}
                      className="flex-1 bg-surface-elevated border border-surface-border rounded-xl px-4 py-3 text-sm text-text-primary placeholder-text-muted focus:border-admin-500 focus:outline-none resize-none"
                    />
                    <button onClick={handleSendReply} disabled={!reply.trim() || sending}
                      className="px-4 rounded-xl bg-admin-500 hover:bg-admin-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold transition-colors flex items-center gap-2 self-end pb-3 pt-3">
                      {sending ? (
