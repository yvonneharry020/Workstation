...
export default function FinanceTicketsPage() {
  ...
  async function handleReturnToManagement() {
    ...
    const returnNote = `[Finance → Management] Returned to management on ${new Date().toLocaleDateString('en-NG')}`
          ...
          {!selected ? (
            <div className="h-full flex items-center justify-center text-text-muted">
              <div className="text-center">
                ...
                <p className="text-sm">Select a ticket to work on it</p>
              </div>
            </div>
          ) : (
            <div className="p-6 space-y-5 max-w-2xl">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  {selected.status !== 'resolved' && selected.status !== 'closed' && (
                    <button onClick={handleResolve} disabled={resolving}
                      className="px-3 py-1.5 rounded-lg bg-finance-500/20 text-finance-400 border border-finance-500/30 text-xs font-semibold hover:bg-finance-500/30 disabled:opacity-40 transition-colors">
              ...
              <div className="bg-surface-card rounded-xl border border-surface-border p-4">
                ...
                <textarea value={internalNote} onChange={e => setInternalNote(e.target.value)}
                  placeholder="Add a finance note…" rows={2}
