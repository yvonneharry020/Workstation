                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(c => (
                    <button key={c} onClick={() => setForm(f => ({ ...f, category: c }))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${form.category === c ? 'bg-finance-500 text-white border-finance-500' : 'bg-surface-elevated border-surface-border text-text-secondary'}`}>
                      {c}
                    </button>
                  ))}
                </div>
