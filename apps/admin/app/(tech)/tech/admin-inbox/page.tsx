...
const STATUS_STYLES: Record<TicketStatus, string> = {
  ...
  in_progress: 'bg-tech-500/15 text-tech-400 border-tech-500/30',
...
export default function TechAdminInboxPage() {
  ...
  useEffect(() => {
    const ch = supabase.channel('tech-admin-inbox')
      ...
      <TopBar
        ...
        actions={
          <button onClick={() => setShowCompose(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-tech-500 hover:bg-tech-600 text-white text-sm font-semibold transition-colors">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-surface-border/50">
            {loading ? (
              ...
              <button key={t.id} onClick={() => setSelectedId(t.id)}
                className={`w-full text-left px-4 py-3.5 transition-colors ${selectedId === t.id ? 'bg-tech-900/30 border-l-2 border-l-tech-500' : 'hover:bg-surface-elevated/40 border-l-2 border-l-transparent'}`}>
          ...
          {!selected ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                <svg className="w-12 h-12 mx-auto mb-3 opacity-20 text-tech-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
            ...
            <div className="px-6 py-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(c => (
                    <button key={c.value} onClick={() => setForm(f => ({ ...f, category: c.value }))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${form.category === c.value ? 'bg-tech-500 text-white border-tech-500' : 'bg-surface-elevated border-surface-border text-text-secondary'}`}>
                      {c.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {(['low','normal','high','urgent'] as const).map(p => (
                    <button key={p} onClick={() => setForm(f => ({ ...f, priority: p }))}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border capitalize transition-colors ${form.priority === p ? 'bg-tech-500/20 text-tech-300 border-tech-500/40' : 'bg-surface-elevated border-surface-border text-text-secondary'}`}>
                      {p}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="text" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} placeholder="Brief summary of your message"
                  className="w-full bg-surface-elevated border border-surface-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-tech-500 focus:outline-none" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleSend} disabled={sending || !form.subject.trim() || !form.description.trim()}
                  className="flex-1 py-2.5 rounded-xl bg-tech-500 hover:bg-tech-600 text-white text-sm font-semibold disabled:opacity-40 transition-colors">
