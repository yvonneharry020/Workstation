...
export default function CompanySupportTicketScreen() {
  ...
  useEffect(() => {
    let channelName = 'company-tickets'
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return
      const ch = supabase
        .channel(`company-tickets-${user.id}`)
        .on('postgres_changes', {
        ...
        .subscribe()
      channelName = `company-tickets-${user.id}`
    })
  ...
  async function handleSubmit() {
    ...
    if (error || !data) {
      ...
      logEvent({ event: 'company.profile_updated', app: 'company_app', targetId: (data as Ticket).id, targetType: 'support_ticket', metadata: { subject: subject.trim(), category, action: 'ticket_submitted' } })
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F5F0E8]" edges={['top']}>
      <View className="px-4 py-3 border-b border-[#DDD6C9] flex-row items-center gap-3">
        ...
        >
          <Text className="text-[#475569] text-lg">←</Text>
        </TouchableOpacity>
      ...
      {view === 'list' ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
          ...
          <View className="flex-row flex-wrap gap-2 mb-4">
            {CATEGORIES.map(c => (
              <TouchableOpacity
                ...
                className={`px-3 py-1.5 rounded-lg border ${category === c.value ? 'bg-[#F59E0B] border-[#F59E0B]' : 'bg-[#13111F] border-[#DDD6C9]'}`}
              >
          ...
          <TouchableOpacity
            ...
            className={`py-4 rounded-xl items-center ${submitting || !subject.trim() || !description.trim() ? 'bg-[#EDE7DB]' : 'bg-[#F59E0B]'}`}
          >
            {submitting ? (
