...
import { AtsRowFull, PIPELINE_CONFIG, PIPELINE_ORDER, PipelineStage } from './types'
...
function buildSlices(rows: AtsRowFull[]): SliceData[] {
  ...
  const activeStages = PIPELINE_ORDER.filter((s) => counts[s] > 0)
...
function AnimatedSlice({ slice, delay }: { slice: SliceData; delay: number }) {
  ...
  const cfg     = PIPELINE_CONFIG[slice.stage]
  ...
  return (
    <Animated.View style={[animStyle, { position: 'absolute', width: SIZE, height: SIZE }]}>
...
function KpiCard({ label, value, sub, color, index }: {
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 11, color: '#9A8FA6', lineHeight: 15 }}>{sub}</Text>
    </Animated.View>
  )
}
  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        paddingVertical: 12,
        borderBottomWidth: 1, borderBottomColor: '#E5DFD3',
        gap: 12,
      }}
    >
      <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: cfg.color, flexShrink: 0 }} />
      <Text style={{ width: 100, fontSize: 13, fontWeight: '600', color: '#1A1625' }}>{cfg.label}</Text>
    ...
    >
      ...
      >
        ...
        <View style={{ width: SIZE, height: SIZE, alignItems: 'center', justifyContent: 'center' }}>
          ...
          >
            <Text style={{ fontSize: 34, fontWeight: '800', color: '#1A1625', letterSpacing: -1 }}>{total}</Text>
      ...
      <Animated.View
        ...
        style={{
          backgroundColor: '#FFFFFF', borderRadius: 18, padding: 18,
          borderWidth: 1, borderColor: '#E5DFD3',
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 13, color: '#5A4F6E' }}>Candidates still in the pipeline</Text>
        </View>
        <Text style={{ fontSize: 36, fontWeight: '800', color: '#6366F1', letterSpacing: -1 }}>{inPipeline}</Text>
      </Animated.View>
      ...
      {slices.length > 0 && (
        <View style={{
          ...
          {slices.map((slice, i) => <LegendRow key={slice.stage} slice={slice} index={i} />)}
        </View>
      )}
      ...
      {total === 0 && (
        <Animated.View entering={FadeInDown.delay(200).duration(300)} style={{ alignItems: 'center', paddingTop: 16 }}>
          ...
          <Text style={{ color: '#9A8FA6', fontSize: 13, textAlign: 'center' }}>
            Analytics will appear as candidates apply for this job.
          </Text>
        </Animated.View>
      )}
    </ScrollView>
